...
#define ROWS 3
static const TxtId LBL[ROWS]  = { T_SIZE, T_START_TEMP, T_RESULT };
...
static const TxtId START_T[2] = { T_FRIDGE, T_ROOM };
static const TxtId DONE_T[3]  = { T_SOFT, T_MEDIUM_EGG, T_HARD };
static const int   COUNT[ROWS] = { 4, 2, 3 };

static const float MASS[4]  = { 48.0f, 58.0f, 68.0f, 78.0f };   // Mitten der Klassen
static const float EGG_START_C[2] = { 4.0f, 20.0f };
static const float T_YOLK[3] = { 63.0f, 70.0f, 80.0f };

static lv_obj_t *s_row[ROWS], *s_val[ROWS], *s_lbl[ROWS], *s_time, *s_btn, *s_title;
...
static uint32_t egg_seconds() {
  ...
  float arg = 0.76f * (EGG_START_C[s_sel[1]] - TW) / (T_YOLK[s_sel[2]] - TW);
...
void ui_egg_create(lv_obj_t *p) {
  ...
  s_btn = make_button(p, "", 200, 54, start_cb, nullptr);
  lv_obj_align(s_btn, LV_ALIGN_CENTER, 0, 108);
  button_set_color(s_btn, lv_palette_main(LV_PALETTE_AMBER));
}

void ui_egg_update() {
  lv_label_set_text(s_title, T(T_EGG));
  button_set_text(s_btn, lang_btn(LV_SYMBOL_PLAY, T_COOK));
  const TxtId *tab[ROWS] = { nullptr, START_T, DONE_T };
  for (int i = 0; i < ROWS; i++) {
    lv_label_set_text(s_lbl[i], T(LBL[i]));
    if (i == 0) lv_label_set_text(s_val[0], SIZE_T[s_sel[0]]);   // Groessen sind Zahlen
    else        lv_label_set_text(s_val[i], T(tab[i][s_sel[i]]));
